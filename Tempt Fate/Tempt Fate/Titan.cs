	...
	public class Titan : Character
	{
		...
		public override void Update(GameTime gameTime, List<Line> Lines, GamePadState gamepadstate, Character enemy)
		{
			...
			catch (ArgumentOutOfRangeException ex)
			{
				...
				var errorfile = File.Create("errorTemptFateTitan");
				errorfile.Close();
				File.WriteAllText("errorTemptFateTitan", ex.Message);
			}
			//Update Bullets
			...
			using (var stream = File.Create("Debugging_VariablesTitan")) { }
			using (StreamWriter nf = new StreamWriter("Debugging_VariablesTitan"))
			{
